...
def build(include_networkx=False):
    ...
    readme_rows = [
        ...
        [f"networkx: {('bundled ' + nx_version + ', used only if none is installed')
                      if nx_version else 'NOT included - must already be installed'}"],
        [f"payload sha256 {digest}"],
        [""],
        ["TIP - do not retype the long line in STEP 2. Click that cell, copy the"],
        ["text from the formula bar, and paste it into the terminal."],
        ["If 'python' is not recognised, try 'py' instead, everywhere below."],
        [""],
        ["STEP 1 - put this file in a folder of its own and open a terminal there"],
        ["   e.g.  mkdir C:\\ITA   then move this .xlsx into it, then:"],
        ["   cd C:\\ITA"],
        [""],
        ["STEP 2 - recover the files (one line, copy it - do not retype):"],
        [BOOTSTRAP],
        ["   expect: no output at all. A new _stage folder appears."],
        [""],
        ["STEP 3 - apply them. SAME COMMAND every time, first install or update:"],
        ["python _stage/unpack.py --target ."],
        [f"   expect: 'applied {len(members)} file(s)' and a list of new files."],
        ["   it refuses and writes nothing if anything arrived damaged."],
        [""],
        ["STEP 4 - FIRST TRANSFER ONLY. Builds the practice map, about a minute:"],
        ["python generate_topology.py"],
        ["   expect: 'circuits: 1000 pre-existing connections'"],
        [""],
        ...
        ["STEP 6 - run it:"],
        ["python server.py"],
        ["   then open Chrome at  http://127.0.0.1:8800"],
        ["   stop it with Ctrl+C"],
        [""],
        ["----------------------------------------------------------------"],
        ["IF SOMETHING GOES WRONG"],
        [""],
        ["STEP 2 gives an error, or _stage is empty:"],
        ["   the file was altered in transfer. Ask for a new one."],
        [""],
        ["STEP 3 says REFUSED:"],
        ["   good - it protected you. Nothing was written. Ask for a new file."],
        [""],
        ...
        [""],
        ["----------------------------------------------------------------"],
        ["ON A LATER TRANSFER, READ THIS"],
        [""],
        ["Run STEP 2 and STEP 3 exactly as above - they are safe to repeat."],
        ["unpack.py never touches data/, so executed circuits and your own"],
        ["zones.json survive, and anything it replaces is copied to _backup_*/"],
        [""],
        ["DO NOT run STEP 4 again. By then the map holds real work or real ITA"],
        ["data, and generate_topology.py would erase it."],
    ]
